...
#SBATCH -o normalization_free_deit_small_patch16_224_layer12_BS1024_Both_Full+BatchNorm+BNFFN_PerOperation_ShortcutGain0.2+FinetuneAfter200Epochs_lr1e-3_warmuplr1e-6+warmup20epochs_minlr1e-5_out.txt
#SBATCH -e normalization_free_deit_small_patch16_224_layer12_BS1024_Both_Full+BatchNorm+BNFFN_PerOperation_ShortcutGain0.2+FinetuneAfter200Epochs_lr1e-3_warmuplr1e-6+warmup20epochs_minlr1e-5_err.txt
...
srun python -m torch.distributed.launch --nproc_per_node=$SLURM_NTASKS_PER_NODE --master_addr=$MASTER_ADDR --master_port=$MASTER_PORT --use_env main.py --data-path /scratch/itee/uqxxu16/data/imagenet --batch-size 256 --model normalization_free_deit_small_patch16_224_layer12 --accumulation-steps=1 --drop-path=0.05 --output_dir=output/normalization_free_deit_small_patch16_224_layer12_BS1024_Both_Full+BatchNorm+BNFFN_PerOperation_ShortcutGain0.2+FinetuneAfter200Epochs_lr1e-3_warmuplr1e-6+warmup20epochs_minlr1e-5 --affected_layers=Both --shortcut_type=PerOperation --feature_norm=BatchNorm --lr=1e-3 --min-lr=1e-5 --warmup-lr=1e-6 --shortcut_gain=0.2 --finetune_gain=200 --opt=nadamw --num_workers=10 --weight-decay=0.05 --warmup-epochs=20